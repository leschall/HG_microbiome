...
tmp=/home/${USER}/rds/rds-mobile2-DZGuFlieARM/TEMP
mem_gb=$(( $threads * 4580 / 1000 ))

# initial mapping and sorting
echo "$(timestamp) [ map2ref pipeline ] Running BWA ..."
rm -f ${outprefix}_${refix}*bam*
bwa-mem2 mem -M -t ${threads} ${ref} ${reads} ${reads2} | samtools view -@ ${threads_sam} -F 256 -uS - | samtools sort -@ ${threads_sam} - -o ${outprefix}_${refix}_raw.bam
samtools index -@ ${threads_sam} ${outprefix}_${refix}_raw.bam
# remove duplicates - choose gatk or picard
#gatk MarkDuplicatesSpark --java-options "-Xms${mem_gb}G -Xmx${mem_gb}G -Djava.io.tmpdir=${tmp}" -I ${outprefix}_${refix}_raw.bam -O ${outprefix}_${refix}_noDup.bam -M ${outprefix}_markdup_metrics.txt --remove-all-duplicates --spark-master local[10]
#picard MarkDuplicates TMP_DIR=${tmp} INPUT=${outprefix}_${refix}_raw.bam OUTPUT=${outprefix}_${refix}_noDup.bam METRICS_FILE=${outprefix}_markdup_metrics.txt REMOVE_DUPLICATES=true MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=500
module load picard/2.9.2
java -Djava.io.tmpdir=${tmp} -Xms${mem_gb}G -Xmx${mem_gb}G -XX:ParallelGCThreads=${threads} -jar ${PICARD_HOME}/picard.jar MarkDuplicates INPUT=${outprefix}_${refix}_raw.bam OUTPUT=${outprefix}_${refix}_noDup.bam METRICS_FILE=${outprefix}_markdup_metrics.txt REMOVE_DUPLICATES=true MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=500
samtools index -@ ${threads_sam} ${outprefix}_${refix}_noDup.bam
tools/bam_ani-filter.py ${outprefix}_${refix}_noDup.bam 90 60 | samtools view -u - -o ${outprefix}_${refix}_sorted.bam
...
rm -rf ${outprefix}_${refix}_raw.ba* ${outprefix}_${refix}_unsorted.bam ${outprefix}_${refix}_sorted.ba* ${outprefix}_${refix}_depth* ${outprefix}_${refix}_noDup.bam*
#rm -rf ${outprefix}_${refix}_unique_depth*
#rm -rf ${outprefix}_${refix}_raw.ba* ${outprefix}_${refix}_noDup.bam* ${outprefix}_${refix}_unsorted.bam ${outprefix}_${refix}_depth*
